function [population,innovation_record] = init_individualRobotNav(PopSize,vector_connected_input_nodes,number_input_nodes,number_output_nodes,ObjNum)
...
number_connections=(length(vector_connected_input_nodes))*number_output_nodes;
vector_connection_from=repmat([vector_connected_input_nodes],[1 number_output_nodes]);
vector_connection_to=[];

for index_output_node = (number_input_nodes+1):(number_input_nodes+number_output_nodes)
    vector_connection_to = [vector_connection_to,index_output_node*ones(1,length(vector_connected_input_nodes))];
end
connection_matrix = [vector_connection_from;
    vector_connection_to];
for index_individual = 1:PopSize
    population(index_individual).nodegenes = [1:(number_input_nodes+number_output_nodes);
        ones(1,number_input_nodes),2*ones(1,number_output_nodes);
        zeros(1,number_input_nodes),zeros(1,number_output_nodes);
        zeros(1,number_input_nodes+number_output_nodes)];
    population(index_individual).connectiongenes = [1:number_connections;
        connection_matrix;
        rand(1,number_connections)-0.5;
    ...
    population(index_individual).Trainfitness = zeros(1,ObjNum);
    population(index_individual).Testfitness = 0;
    population(index_individual).TrainfitnessNorm=zeros(1,ObjNum);
end
