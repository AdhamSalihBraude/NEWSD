function population_plus_fitnesses = RobotNavcalcfitNEAT(population, Robot, Arena,pltrobot,weights)
    ...
    for IndexArena = 1 : length(Arena)
        if weights(IndexArena) ~= 0
            ...
            for step  = 1 : Arena(IndexArena).MaxStep
                ...
                %             wMotors = NetCalc(Ind,Input,2)./[2,2*Robot.WheelDiameter];
            ...
            Trainfitness(IndexArena) = 1;
        end
    end
    Trainfitness = (Trainfitness.*(1-wallhit));
    population_plus_fitnesses(index_individual).Trainfitness = Trainfitness;
    population_plus_fitnesses(index_individual).fitness = Trainfitness*weights'/sum(weights);
