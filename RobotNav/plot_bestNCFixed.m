function plot_bestNCFixed(population, Robot, Arena,pltrobot,Vid)
...
for index_individual=1:number_individuals
    Ind = population(randi(number_individuals));    
    ...
    for IndexArena = 1 : length(Arena)
        ...
        for step  = 1 : Arena(IndexArena).MaxStep
            ...
            NNInput = [1-tar_sens,1-obs_sens]';%,wMotors];
            %             wMotors = NetCalc(Ind,Input,2)./[2,2*Robot.WheelDiameter];
            HN = [];
            for L = 2 : length(Ind.Layers)
                W = Ind.Layers(L).W;
                sigma = W'*NNInput;
                HN = Ind.Layers(L).f(sigma);
                NNInput = HN;
            end
            wMotors = HN - 0.5;
            %             %             wMotors = [1 0.5];
